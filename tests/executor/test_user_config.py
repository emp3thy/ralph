...
def test_read_rejects_non_string_workspace_root(fake_home: Path) -> None:
    cfg = fake_home / ".ralph" / "config.toml"
    cfg.parent.mkdir(parents=True)
    cfg.write_text("workspace_root = 42\n", encoding="utf-8")
    with pytest.raises(ConfigError, match="workspace_root must be a non-empty string"):
        read_workspace_root()


def test_read_rejects_empty_string_workspace_root(fake_home: Path) -> None:
    cfg = fake_home / ".ralph" / "config.toml"
    cfg.parent.mkdir(parents=True)
...
def test_read_wraps_oserror_as_configerror(
    ...
    cfg = fake_home / ".ralph" / "config.toml"
    cfg.parent.mkdir(parents=True)
    cfg.write_text('workspace_root = "/x"\n', encoding="utf-8")
...
def test_read_raises_on_malformed_toml(fake_home: Path) -> None:
    cfg = fake_home / ".ralph" / "config.toml"
    cfg.parent.mkdir(parents=True)
    cfg.write_text("workspace_root = ===bogus===\n", encoding="utf-8")
...
def test_write_escapes_control_characters_in_path(fake_home: Path) -> None:
    ...
    write_workspace_root(weird)
    # Critical: re-reading must NOT raise (file is valid TOML).
    assert read_workspace_root() == weird
...
def test_read_expands_user_in_value(fake_home: Path) -> None:
    ...
    write_workspace_root(Path("~/dev/ralph"))
    assert read_workspace_root() == Path("~/dev/ralph").expanduser()
