...
## Iteration 9 — 2026-05-27T03:51:00+00:00

- Step: Task 9 — regression coverage for two-worktree execution. New `tests/executor/test_worktree.py` covers the helpers (`ensure_worktree` new/idempotent/branch-from-base, `remove_worktree` cleans-up-and-prunes/tolerates-missing) against a real local git repo. New tests in `tests/executor/test_loop.py` cover the worktree-mode integration: claim creates queue + work trees without touching the primary checkout's branch; `_persist_iteration_writes` commits from the queue worktree and pushes to `origin/ralph-queue` while primary HEAD stays put; terminal `pr_created` outcome tears down the work worktree but preserves the queue worktree and the `ralph/<id>` ref. New tests in `tests/executor/test_claude_spawn.py` cover the `cwd` / `pbi_dir` overrides (echoed back via the fake claude script) and the fail-fast `FileNotFoundError` when `pbi_dir` does not exist.
- Bugs surfaced and fixed by the new tests:
  - `ralph_executor/worktree.py::list_worktrees` parsed porcelain entries with key `worktree` (from `key, sep, value = line.partition(" ")`) but the rest of the module assumed `path`. `_existing_worktree_paths` always returned an empty set, so `ensure_worktree`'s idempotency check was a no-op and a second call would re-attempt `git worktree add`. Fix: normalise `worktree` → `path` in the post-processing pass alongside the existing `branch` strip-prefix step. Memory id `a3b36bee3d724b928537c301ad5e62fe`.
  - `ralph_executor/queue/filesystem.py::FilesystemQueueSource._root` used `cfg.repo_path / .ralph` unconditionally. In worktree mode the queue lives at `cfg.repo_path/.ralph-work/queue/.ralph/`, so `pick_next()` always returned `None` and `iterate_once` always reported `idle`. Fix: route through `queue_worktree_path` when `cfg.use_worktrees`. Memory id `fe16c38df4cf4e5dbf32b2c7ab623115`.
- Design notes:
  - Helper tests fixture (`worktree_repo`) initialises a minimal local git repo (no bare remote, no `ralph-queue`) — the helpers don't need that machinery and a smaller fixture isolates the parser/idempotency assertions from queue concerns.
  - Loop-integration fixture (`cfg_for_repo_worktree`) is a `dataclasses.replace(cfg_for_repo, use_worktrees=True)` shim with a best-effort `git worktree remove --force` + `prune` teardown so pytest's tmp_path cleanup doesn't trip on Windows directory pinning (memory id `0dda832cbec346c29e5e81e3cb2113f9`).
  - Skipped writing a dedicated `test_legacy_single_checkout_path_still_works_when_flag_false` — the entire existing `test_loop.py` suite uses `cfg_for_repo` (use_worktrees=False) and exercises claim → spawn → persist → terminal-outcome moves end-to-end, so the legacy path already has dense regression coverage. Noted explicitly at the bottom of the new test block.
  - Path comparison helper (`_registered_worktree_paths`) resolves porcelain-emitted paths via `Path` so the assertion holds on Windows where porcelain emits forward slashes but `Path.resolve()` returns backslashes.
