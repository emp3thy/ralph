...
## Iteration 9 — 2026-05-28T05:30:00Z

- Step: Task 7 Step 7.8 — `iterate_once` now catches `_ClaimError` from `_claim_pbi`, appends a `## Claim failed — <ISO>` block (with the failure reason) to the PBI's `HISTORY.md`, and demotes it from `inbox/` to `blocked/<id>/` via the new `move_inbox_to_blocked` movement helper. New `_move_to_blocked_with_reason` helper in `loop.py` orchestrates the append-then-move (single commit, HISTORY.md appended BEFORE `git mv` so the move's commit captures both). Added `"claim_failed"` to the `IterationOutcome` literal; the catch path returns `IterationResult(outcome="claim_failed", pbi_id=…)` and skips the cycle-detector (move is final, not a retry).
- Tests: 2 new tests in test_loop.py (`test_iterate_once_moves_pbi_to_blocked_when_claim_raises_claim_error` — legacy mode + non-github host; `test_iterate_once_moves_pbi_to_blocked_when_target_unreachable_in_worktree_mode` — worktree mode + monkeypatched TargetUnreachable). Both green. Full `tests/executor tests/safety` sweep 471/471 green.
- Lint: ruff check + ruff format clean on changed files; mypy --strict on loop.py + queue/movements.py clean.
- Notes: `move_inbox_to_blocked` mirrors `move_current_to_blocked` — same `_move(...)` plumbing, just `expected_state="inbox"`. No event emission (PBI_BLOCKED is reserved for the post-attempts blocked path in `_run_ralph`; claim_failed is a different signal). HISTORY.md is created when absent (defensive — the PBI directory schema requires it, but a malformed inbox PBI shouldn't mask the real claim failure with a write error). Task 7 is now complete (sub-steps 7A/7B/7C + Step 7.8 all checked). Next iteration: Task 8 — `claude_spawn` cwd + `GH_OWNER` per-subprocess.
