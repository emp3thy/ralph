...
> **HISTORICAL REFERENCE (Stage B era).** Stage B is complete: the operator skills this runbook works around (`ralph-new`, `ralph-status`, `ralph-cancel`, `ralph-promote`, `ralph-triage`, `ralph-recover`) have shipped, queue provisioning is `scripts/setup_ralph_queue_github.py`, and the executor is queue-driven — each PBI's `target_repo` frontmatter names the repo; the `RALPH_REPO_PATH` env var no longer exists. Use this document only as the "operate without the tools" fallback described in §8. For current architecture see `docs/runbooks/ralph-architecture.md`.
