...
- [x] 1. Write `tests/executor/sweep/test_decide_action.py` with the exact content below. Every test instantiates a `PrSnapshot` directly — no I/O, no subprocess. This is the single source of truth for the sweep table.
  ```python
...
- [x] 2. Run the tests; they must fail because `SweepConfig` and `decide_action` do not yet exist:
  ```
...
- [x] 3. Do NOT commit yet — the failing tests are consumed by Task 3.
...
- [x] 1. Create `ralph_executor/sweep/runner.py` containing exactly the content below. The orchestration parts (`run`, `SweepResult`) are stubbed so imports work; they are completed in Task 6. The pure function `decide_action` is fully implemented now to turn the failing tests green:
  ```python
...
- [x] 2. Run the failing tests; they must now pass:
  ```
  uv run pytest tests/executor/sweep/test_decide_action.py -v
  ```
...
- [x] 1. Write `tests/executor/sweep/test_pr_state.py` first (red step). The test invokes `pr_state.fetch` with a `skill_path` pointing at a small fake script that prints canned JSON to stdout, simulating the real `ado-pr` skill scripts:
  ```python
...
- [x] 2. Run the failing tests to confirm they fail because `pr_state.py` does not exist:
  ```
...
- [x] 3. Create `ralph_executor/sweep/pr_state.py` containing exactly:
  ```python
...
- [x] 4. Run the failing tests; they must now pass:
  ```
  uv run pytest tests/executor/sweep/test_pr_state.py -v
  ```
  Expected: every test in `test_pr_state.py` passes.

- [x] 5. Run static checks:
  ```
  uv run ruff check ralph_executor/sweep tests/executor/sweep
  uv run mypy ralph_executor
  ```
