...
def _parse_int(name: str, default: int) -> int:
    ...
    try:
        ...
        raise ConfigError(f"{name}={raw!r} is not an integer: {exc}") from exc
...
def _parse_float(name: str, default: float) -> float:
    ...
    try:
        ...
        raise ConfigError(f"{name}={raw!r} is not a float: {exc}") from exc
...
def _parse_log_level(name: str, default: str) -> int:
    ...
    if raw not in _VALID_LOG_LEVEL_NAMES:
        raise ConfigError(f"{name}={raw!r} not in {sorted(_VALID_LOG_LEVEL_NAMES)}")
...
def load_config() -> ExecutorConfig:
    ...
    # OAuth session when no key is provided. We propagate the key into the
    # subprocess env in claude_spawn only if it was actually supplied.
    ...
    sleep_seconds = _parse_float("RALPH_ITERATION_SLEEP_SECONDS", DEFAULT_ITERATION_SLEEP_SECONDS)
    claude_binary = os.environ.get("RALPH_CLAUDE_BINARY") or DEFAULT_CLAUDE_BINARY
