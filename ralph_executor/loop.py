...
def _queue_repo_root(cfg: ExecutorConfig) -> Path:
    ...
    Resolves to ``<workspace_root>/queue-<instance_id>``. The queue repo
    is cloned by ``ensure_queue_clone`` into that directory and owns
    ``.ralph/`` (events.db, sentinel, blocked/, …). Every operation that
    reads or writes under ``.ralph/`` — opening the event log, moving
    PBIs to ``.ralph/blocked/``, handling STUCK.md, checking/writing the
    halt sentinel — routes through this helper so the side-effects land
    in the queue clone that gets pushed to ``origin/<queue_branch>`` of
    ``queue_repo``.
    """
