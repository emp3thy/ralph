...
pushes the queue branch (`cfg.queue_branch`, default: `ralph-queue`)
to `origin`. Ralph's loop reads `CANCEL` at the top of
...
- It does not delete any PBI files. The cancellation is recorded via
  the empty `CANCEL` file plus a commit on the queue branch
  (`cfg.queue_branch`, default: `ralph-queue`) in the queue repo.
